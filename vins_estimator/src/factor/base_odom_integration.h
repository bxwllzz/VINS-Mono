...
public:
    explicit BaseOdometryIntegration3D(const Eigen::Matrix3d& _scale = Eigen::Matrix3d::Identity(), const Eigen::Vector3d& _bg = Eigen::Vector3d::Zero());

    void push_back(const MixedOdomMeasurement& m);

    void propagate(const MixedOdomMeasurement& m);

    void repropagate(const Eigen::Matrix3d& _scale, const Eigen::Vector3d& _bg);
